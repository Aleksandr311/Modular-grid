...
let string = 'Hi';
string = 'h' + string[1]; // заменяем строку
...
//Сравнение строк: строки сравниваются посимвольно в алфавитном порядке.
//Тем не менее, есть некоторые нюансы. Строчные буквы больше заглавных:
...
console.log( "pizza".codePointAt(2) ); // 122
console.log( "PIZZA".codePointAt(3) ); // 90
...
//с шестнадцатеричным кодом символа: 90 — 5a в шестнадцатеричной системе счисления
console.log( '\u005a' ); // Z
//сделаем строку, содержащую символы с кодами от 65 до 220 — это латиница и ещё некоторые
...
//все современные браузеры поддерживают стандарт ECMA 402, обеспечивающий правильное
...
//0, если строки равны.
console.log( 'Москва'.localeCompare('Томск') ); // -1
console.log( 'Мел'.localeCompare('мел') ); // 1
//У этого метода есть два дополнительных аргумента, которые указаны в документации. 
//Первый позволяет указать язык (по умолчанию берётся из окружения) — от него зависит
//порядок букв. Второй — определить дополнительные правила, такие как чувствительность
//к регистру, а также следует ли учитывать различия между "a" и "á".

//Диакритические знаки и нормализация, суррогатные пары, рано знать так сказано))
